...
for(let i = 0; i < img.length; i++){
  let images = new Image(100); // only width, for save original view
  images.className = "group";
  images.id = `${img[i].id}`;
  images.setAttribute("name", `${img[i].name}`);
  images.src = img[i].imageUrl;
  
  field.appendChild(images); //  add in the field all images

  images.addEventListener("click", function(){
    images.setAttribute("class", "selected");   
    let id = images.getAttribute("id");
    let name = images.getAttribute("name");
    //alert(`id: ${images.getAttribute("id")}, name: ${images.getAttribute("name")}`); 
   // we can confirm our choice, but after according to the conditions
    document.getElementById('subBut')
